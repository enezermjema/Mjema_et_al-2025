...
cor_winter <- cor(df_winter_spieke2[, c(1:4)], # removing latitude
                  method = "spearman", use = "pairwise.complete.obs")
## spring spiekeroog
cor_spieke_2021 <- cor(df_spring_spieke2[df_spring_spieke2$year == 2021, c(1:10)], 
                       method = "spearman", use = "pairwise.complete.obs")
cor_spieke_2022 <- cor(df_spring_spieke2[df_spring_spieke2$year == 2022, c(1:10)], 
                       method = "spearman", use = "pairwise.complete.obs")
cor_spieke_2023 <- cor(df_spring_spieke2[df_spring_spieke2$year == 2023, c(1:10)], 
                       method = "spearman", use = "pairwise.complete.obs")
cor_spieke_2024 <- cor(df_spring_spieke2[df_spring_spieke2$year == 2024, c(1:10)], 
                       method = "spearman", use = "pairwise.complete.obs")
cor_spieke_2025 <- cor(df_spring_spieke2[df_spring_spieke2$year == 2025, c(1:10)], 
                       method = "spearman", use = "pairwise.complete.obs")

## spring brachwitz
cor_brachwitz_2023 <- cor(df_spring_brachwitz2[df_spring_brachwitz2$year == 2023, c(1:10)], 
                          method = "spearman", use = "pairwise.complete.obs")
cor_brachwitz_2024 <- cor(df_spring_brachwitz2[df_spring_brachwitz2$year == 2024, c(1:10)], 
                          method = "spearman", use = "pairwise.complete.obs")
cor_brachwitz_2025 <- cor(df_spring_brachwitz2[df_spring_brachwitz2$year == 2025, c(1:10)], 
                          method = "spearman", use = "pairwise.complete.obs")

# Computing pvalues

# spiekeroog samples
p_winter <- cor_pmat(df_winter_spieke2[, c(1:4)], 
                     method = "spearman", )

p_spieke_2021 <- cor_pmat(df_spring_spieke2[df_spring_spieke2$year == 2021, c(1:10)], 
                          method = "spearman")
p_spieke_2022 <- cor_pmat(df_spring_spieke2[df_spring_spieke2$year == 2022, c(1:10)], 
                          method = "spearman")
p_spieke_2023 <- cor_pmat(df_spring_spieke2[df_spring_spieke2$year == 2023, c(1:10)], 
                          method = "spearman")
p_spieke_2024 <- cor_pmat(df_spring_spieke2[df_spring_spieke2$year == 2024, c(1:10)], 
                          method = "spearman")
p_spieke_2025 <- cor_pmat(df_spring_spieke2[df_spring_spieke2$year == 2025, c(1:10)], 
                          method = "spearman")

# spring brachwitz
p_brachwitz_2023 <- cor_pmat(df_spring_brachwitz2[df_spring_brachwitz2$year == 2023, c(1:10)], 
                             method = "spearman")
p_brachwitz_2024 <- cor_pmat(df_spring_brachwitz2[df_spring_brachwitz2$year == 2024, c(1:10)], 
                             method = "spearman")
p_brachwitz_2025 <- cor_pmat(df_spring_brachwitz2[df_spring_brachwitz2$year == 2025, c(1:10)], 
                             method = "spearman")
...
ggsave(filename = "Desktop/git/Chapter 1/plots/correlation-spiekeroog (no lat).pdf", 
       plot = spieke_all, width = 19, height = 11, units = "in", dpi = 450)
ggsave(filename = "Desktop/git/Chapter 1/plots/correlation-brachwitz (no lat).pdf", 
